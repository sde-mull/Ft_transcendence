import { updatePathToModuleMap } from "../../../Content.js";
import { attachEventListener } from '../../../Tools/CreateEventListeners.js';
import { handleNavigation } from '../../../router.js';
import { HandleGameRoom } from "./game_room.js";
...
export const loadContent = async () => {
    ...
    if (result.length > 0) {
        GameListHtml += '<ul>';
        result.forEach(game => {
            GameListHtml += `<li><a href="/Play/Curve/${game.fields.name}" class="game-link">${game.fields.name}</a></li>`;
            updatePathToModuleMap("/Play/Curve/" + game.fields.name, './Game/Curve/Pages/game_room.js');
        });
        GameListHtml += '</ul>';
    } else {
    ...
    attachEventListener('Class', 'game-link', 'click', (event) => HandleGameRoom(event, result));
}
